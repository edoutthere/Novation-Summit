// Summit Patch Breakout   (Windows version)
//
...
int main(int argc, char **argv)
{
	...
	printf("PatchBreakout  Built 01/16/2022\n");
	printf("input: %s\n", argv[1]);
    ...
    while(1)
    	...
    	patchFile = _open( (const char *)patchName, O_CREAT|O_RDWR, 438);
    	if (patchFile == -1)
        	{
            printf("%s\n", strerror(errno) );
       	...
       	_write(patchFile, patchPtr, PATCH_SIZE);
        //printf("write: %s\n", strerror(errno) );
    	_close(patchFile);
